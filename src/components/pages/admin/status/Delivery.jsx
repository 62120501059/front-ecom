...
const Verify = () => {
  ...
  const loadData = async (authtoken) => {
    ...
    await getOrderbyAdmin(authtoken)
      .then((res) => {
        // setData(res.data);
        setData(res.data.filter((order) => order.orderstatus === "4"));
        setDataLen(
          res.data.filter((order) => order.orderstatus === "4").length
        );
      })
      .catch((err) => console.log(err));
  };
  ...
  const handleEdit = async (item) => {
    ...
    // console.log("handleEdit", item.QRimg);
  ...
  const handleSubmitEdit = async (e) => {
    ...
    editData.image2 = values;

    // console.log("Submit", editData);
    // console.log("Value", values);
    updateOrderbyAdmin2(authtoken, id, editData)
      .then((res) => {

...
const Verify = () => {
  ...
  const loadData = async (authtoken) => {
    ...
    await getOrderbyAdmin(authtoken)
      .then((res) => {
        // setData(res.data);
        setData(res.data.filter((order) => order.orderstatus === "5"));
        setDataLen(
          res.data.filter((order) => order.orderstatus === "5").length
        );
      })
      .catch((err) => console.log(err));
  };
            ...
            <div className="overflow-y-scroll max-h-96 mt-2">
              <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                  <td className="w-32 p-4">{item.index}</td>
                </tr>
              ))} */}
              </table>
            </div>

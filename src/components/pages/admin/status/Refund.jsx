...
const Verify = () => {
  ...
  const loadData = async (authtoken) => {
    ...
    await getOrderbyAdmin(authtoken)
      .then((res) => {
        // setData(res.data);
        setData(res.data.filter((order) => order.orderstatus === "7"));
        setDataLen(
          res.data.filter((order) => order.orderstatus === "7").length
        );
      })
      .catch((err) => console.log(err));
  };
  ...
  const handleEdit = async (item) => {
    ...
    getProductByAdmin(authtoken, item._id, item.products)
      .then(async (res) => {
        ...
        // console.log("editData", item.images2[0].url);
        ...
        await setEditData({
          ...
          image2: item.images2[0]?.url || null,
          products: res.data,
